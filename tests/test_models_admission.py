...
from conftest import FIXED_TIME, TEST_AUTHORITY, make_raw_segment, make_segment
...
def test_verified_principal_provenance_maps_each_load_mode() -> None:
    policy = AdmissionPolicy(TEST_AUTHORITY)
...
def test_raw_caller_principal_claim_is_rejected_without_verifier_attestation() -> None:
    ...
    decision = AdmissionPolicy(TEST_AUTHORITY).admit_unverified(raw)
...
def test_empty_allowlist_is_deny_all() -> None:
    policy = AdmissionPolicy(TEST_AUTHORITY, allowed_trust_classes=frozenset())
...
def test_expired_and_wrong_scope_attestations_are_rejected() -> None:
    ...
    assert AdmissionPolicy(TEST_AUTHORITY).admit(expired, now=FIXED_TIME).reason_code is (
        AdmissionReason.PROVENANCE_INVALID
    )
    assert AdmissionPolicy(wrong_scope_authority).admit(valid, now=FIXED_TIME).reason_code is (
        AdmissionReason.PROVENANCE_INVALID
    )
...
def test_batch_rejects_duplicate_logical_ids_and_duplicate_uids() -> None:
    ...
    with pytest.raises(ValueError, match="duplicate segment_id"):
        AdmissionPolicy(TEST_AUTHORITY).admit_many((first, second))
    ...
    with pytest.raises(ValueError, match="duplicate segment_uid"):
        AdmissionPolicy(TEST_AUTHORITY).admit_many((shared, copied_uid))
...
def test_post_verification_content_substitution_is_rejected() -> None:
    ...
    decision = AdmissionPolicy(TEST_AUTHORITY).admit(substituted)
